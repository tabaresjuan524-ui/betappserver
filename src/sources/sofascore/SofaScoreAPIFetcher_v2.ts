...
export class SofaScoreAPIFetcher {
    ...
    /**
     * Fetch event count via direct axios call
     */
    private async fetchEventCount(): Promise<EventCount> {
        try {
            console.log('📊 [SOFASCORE] Fetching event count...');
            const response = await this.axiosInstance.get('/sport/-18000/event-count');
            const sportsWithLive = Object.keys(response.data).filter(s => response.data[s].live > 0);
    ...
    /**
     * Fetch live events for a sport via direct axios call
     */
    private async fetchLiveEvents(sportName: string): Promise<LiveEventData> {
        try {
            ...
            const response = await this.axiosInstance.get(`/sport/${sportName}/events/live`);
            console.log(`✅ [SOFASCORE] ${sportName}: ${response.data.events.length} live events`);
            return response.data;
        } catch (error: any) {
    ...
    private async processEventsForSport(sportName: string, liveEvents: LiveEventData): Promise<any> {
        ...
        // Open tabs for each event (browser pool handles distribution and limits)
        const promises = events.map(async (event) => {
            try {
                ...
                console.error(`❌ [SOFASCORE] Error opening tab for event ${event.id}:`, error);
            }
        });
